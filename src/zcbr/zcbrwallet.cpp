...
CzCBRWallet::CzCBRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcbr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCBRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCBRSeed_deprecated()) {
                    LogPrintf("%s: Updated zCBR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcbr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcbr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCBRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCBRCount(nCountLastUsed);
    else if (!walletdb.ReadZCBRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCBRWallet::Lock()
{
    seedMaster = 0;
}

void CzCBRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCBRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCBR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCBRWallet::LoadMintPoolFromDB()
{
...
void CzCBRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCBRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCBRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcbrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCBRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCBR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcbrTracker which also adds to database
    pwalletMain->zcbrTracker->Add(dMint, true);
...
void CzCBRWallet::SeedToZCBR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCBRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCBRWallet::GenerateDeterministicZCBR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCBRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCBRWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCBRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
