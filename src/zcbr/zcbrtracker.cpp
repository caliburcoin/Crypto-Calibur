...
CzCBRTracker::CzCBRTracker(std::string strWalletFile)
{
...
CzCBRTracker::~CzCBRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCBRTracker::Init()
{
...
bool CzCBRTracker::Archive(CMintMeta& meta)
{
...
bool CzCBRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCBRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCBRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCBRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCBRTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCBRTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCBRTracker::GetSerialHashes()
{
...
CAmount CzCBRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCBRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCBRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCBRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCBRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCBRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCBRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCBRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCBRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCBRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCBRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCBRWallet* zCBRWallet)
{
    bool iszCBRWalletInitialized = (NULL != zCBRWallet);
    CMintMeta meta;
    ...
    if (! iszCBRWalletInitialized)
        zCBRWallet = new CzCBRWallet(strWalletFile);
    meta.isSeedCorrect = zCBRWallet->CheckSeed(dMint);
    if (! iszCBRWalletInitialized)
        delete zCBRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCBRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCBRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCBRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCBRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCBRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCBRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCBRWallet* zCBRWallet = new CzCBRWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zCBRWallet);
        }
        delete zCBRWallet;
        LogPrint("zero", "%s: added %d dzcbr from DB\n", __func__, listDeterministicDB.size());
    }
