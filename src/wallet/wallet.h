...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zcbrspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 20000 CBR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCBR send
enum ZerocoinSpendStatus {
    ZCBR_SPEND_OKAY = 0,                            // No error
    ZCBR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCBR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCBR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCBR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCBR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCBR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCBR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCBR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCBR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCBR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCBR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCBR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCBR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCBR_SPENT_USED_ZCBR = 14,                      // Coin has already been spend
    ZCBR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCBR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    string GetUniqueWalletBackupName(bool fzcbrAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzCBRTracker> zcbrTracker;
    ...
    void setZWallet(CzCBRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zcbrTracker = std::unique_ptr<CzCBRTracker>(new CzCBRTracker(strWalletFile));
    }

    CzCBRWallet* getZWallet() { return zwalletMain; }
    ...
    /** zCBR reset */
    boost::signals2::signal<void()> NotifyzCBRReset;
