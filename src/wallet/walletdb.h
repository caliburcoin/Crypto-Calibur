...
public:
    ...
    bool WriteZCBRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCBRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCBRSeed_deprecated(uint256& seed);
    bool EraseZCBRSeed();
    bool EraseZCBRSeed_deprecated();

    bool WriteZCBRCount(const uint32_t& nCount);
    bool ReadZCBRCount(uint32_t& nCount);
