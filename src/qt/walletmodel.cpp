...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered calibur address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CBR.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
        ...
        // reject insane fee
        if (nFeeRequired > ::minRelayTxFee.GetFee(transaction.getTransactionSize()) * 20000)
            return InsaneFee;
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal calibur:URI (calibur:XyZ...?message=example)
            {
...
static void NotifyzCBRReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCBRReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCBRReset.connect(boost::bind(NotifyzCBRReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCBRReset.disconnect(boost::bind(NotifyzCBRReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcbrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
