...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CBR);
    ...
    if (!settings.contains("nAnonymizeCaliburAmount"))
        settings.setValue("nAnonymizeCaliburAmount", 1000);

    nAnonymizeCaliburAmount = settings.value("nAnonymizeCaliburAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCaliburAmount"))
        SoftSetArg("-anonymizecaliburamount", settings.value("nAnonymizeCaliburAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in calibur.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCaliburAmount:
            return QVariant(nAnonymizeCaliburAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeCaliburAmount:
            nAnonymizeCaliburAmount = value.toInt();
            settings.setValue("nAnonymizeCaliburAmount", nAnonymizeCaliburAmount);
            emit anonymizeCaliburAmountChanged(nAnonymizeCaliburAmount);
            break;
        case CoinControlFeatures:
