...
QString CoinControlDialog::getPriorityLabel(double dPriority, double mempoolEstimatePriority)
{
    ...
    if (dPriority / 1000000 > dPriorityMedium)
        ...
        return tr("higher");
    else if (dPriority / 20000 > dPriorityMedium)
        return tr("high");
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::CBR;
    ...
    QString toolTip4 = tr("Can vary +/- %1 ucbr per input.").arg(dFeeVary);
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show CALIBUR address. In tree mode, address is not shown again for direct wallet address outputs
