...
#include "ui_zcbrcontroldialog.h"
...
std::set<std::string> ZCBRControlDialog::setSelectedMints;
std::set<CMintMeta> ZCBRControlDialog::setMints;

bool CZCBRControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZCBRControlDialog::COLUMN_DENOMINATION || column == ZCBRControlDialog::COLUMN_VERSION || column == ZCBRControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZCBRControlDialog::ZCBRControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCBRControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCBRControlDialog::~ZCBRControlDialog()
{
    delete ui;
}

void ZCBRControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCBRControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZCBRControlWidgetItem* itemDenom(new CZCBRControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZCBRControlWidgetItem *itemMint = new CZCBRControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zcbrTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcbrTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCBR.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCBR seed used to mint this zCBR is not the same as currently hold in the wallet";
            else
...
void ZCBRControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCBRControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCBR_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCBRControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCBRControlDialog::GetSelectedMints()
{
...
void ZCBRControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
